			...
			switch (last_field) {
				case 'G':
					...
					if (DEBUG_ECHO && (debug_flags & DEBUG_ECHO))
						printf("%d", next_target.G);
					break;
				case 'M':
					next_target.M = read_digit.mantissa;
					if (DEBUG_ECHO && (debug_flags & DEBUG_ECHO))
						printf("%d", next_target.M);
					break;
				case 'X':
	...
	if ((c == 10) || (c == 13)) {
		...
		if (
			...
			if (
				...
				// process
				uint32_t tempa = (uint32_t)(30+(1.3934-1.2*((double)ADC_results[0]/(double)ADC_results[1]))/0.0043);
				printf("ok sd:%d, su:%u, T:%u, VREF:%u", RxCounter_SW_DOWN, RxCounter_SW_UP,
						ADC_results[0], tempa);
				...
				printf("rs N%u Expected checksum %u\n", next_target.N_expected, next_target.checksum_calculated);
 				request_resend();
			}
		}
		else {
			printf("rs N%u Expected line number %u\n\r", next_target.N_expected, next_target.N_expected);
 			request_resend();
		}
